...
export function getSearchRegions(regions) {
    return Promise.all(regions.map(getRegionById)).then(regions => {
        const states = regions.filter(region => region && !region.cityCode);
        const cities = regions.filter(region => region && region.cityCode);
        const parentCities = regions.filter(region => region && region.cityCode && region.cityCode.endsWith('0'));
